  ...
  return (
    ...
    >
      <div className="bg-modalBg rounded-t rounded-b shadow-md w-[40%] flex flex-col">
        ...
        <button onClick={handleOpenContact} className=' p-1 rounded w-[25%] bg-channleModalBtnBg hover:bg-channelModalBtnHoverBg text-channelModalBtnTxt text-sm font-[500]'>Select Members</button>
        </div>
        {
          isOpenContact && (
             <div className={`${styles.contactsContainer} border ml-16 `}>
            <h2 className=" font-[600] text-sm text-modalOutBg bg-channleModalContactBg p-2 border w-full">Contacts</h2>
              {Object.keys(groupedUsers).map((letter) => (
                <div key={letter} className='bg-channelModalContactBodyBg'>
                  ...
                  {groupedUsers[letter].map((user) => (
                    <div key={user.id} className={styles.userItem}>
                     <input type="checkbox" className='mr-3 accent-channelModalCheckBoxBg ' />
